...
import { getProjects, toProjectViewModel } from '@utils/api';
...
function ogCard(title: string, description: string | undefined, type: string) {
  ...
  const authorLine = `${site.author.name}${type === 'post' ? ' · Blog' : type === 'project' ? ' · Proyectos' : ''}`;
