...
import type { SocialLink } from '@events/domain/social-link.entity.ts';

export class ListEventSocialLinksUseCase {
  constructor(private readonly events: EventRepository) {}

  async execute(eventId: string): Promise<Result<SocialLink[], AppError>> {
    try {
      ...
      return ok(await this.events.listSocialLinks(eventId));
    } catch (cause) {
      return fail(
        createAppError(CommonErrorCode.DatabaseError, eventErrorTypeByCode, {
          message: 'Failed to list event social links',
          cause
        })
      );
    }
